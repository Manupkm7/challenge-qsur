  ...
  return (
    ...
    >
      <div className="w-6 h-6">
        {value ? <RadioButtonCheckIcon color="#279AF1" /> : <RadioButtonUncheckIcon />}
      </div>
      ...
      className={
        props.selected
          ? `${
              props.selectedStyle
                ? props.selectedStyle
                : `flex border-2 items-center justify-center relative min-w-6 min-h-6 rounded-full pt-6px ${
...
export const RadixRadioGroup = ({ value, items, onChange = () => {} }: RadixRadioGroupProps) => {
  ...
  return (
    <RadioGroupRoot
      ...
      className={`flex flex-row gap-20px w-full`}
    >
          ...
          {items.map(({ value, label, disabled }, index) => (
            <RadioOption key={index}>
              <RadioItem
                ...
                value={value}
                disabled={disabled}
                data-testid={`radio-${currentValue === value ? 'checked' : 'unchecked'}`}
