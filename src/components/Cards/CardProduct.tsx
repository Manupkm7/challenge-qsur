...
export function CardProduct({
  ...
  if (viewMode === 'list') {
    return (
      <div
        className="overflow-hidden transition-all hover:shadow-md cursor-pointer border-2 rounded-lg"
        onClick={handleCardClick}
      >
        <div className="flex">
          {/* Image area */}
          <div
            className="relative w-24 sm:w-32 sm:h-32 flex-shrink-0"
          ...
          <div className="flex flex-col flex-1 p-4">
            <div className="flex items-center justify-between mb-2">
              <div>
                <h2 className="font-semibold" data-testid="title">
                  {title}
                </h2>
                {price !== undefined && (
                  <div className="text-sm font-medium text-primary" data-testid="price">
                    {formatPrice(price)}
                  </div>
                )}
                <div className="text-sm font-medium" data-testid="stock-quantity">
                  Stock: {quantity}
                </div>
              </div>
            ...
            <div className="text-xs text-muted-foreground mt-2" data-testid="created-date">
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-t-2">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <h2 className="font-semibold text-lg leading-none" data-testid="title">
              {title}
            </h2>
            <span className="text-muted-foreground text-lg leading-none" data-testid="price">
              {price ? formatPrice(price) : 'Fuera de stock'}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Badge
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="text-sm font-[600]" data-testid="stock-quantity">
            Stock: {quantity}
          </div>
        </div>
      </div>
      ...
      <div className="border-t-2 bg-muted/30 px-4 py-2">
        <div className="text-xs text-muted-foreground" data-testid="created-date">
          Creado: {formatDate(createdAt)}
        </div>
      </div>
    </div>
