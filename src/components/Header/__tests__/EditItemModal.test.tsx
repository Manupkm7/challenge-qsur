...
describe('EditItemModal', () => {
  ...
  describe('Renderizado inicial', () => {
    it('renderiza correctamente con los datos del producto', () => {
      ...
      expect(screen.getByTestId('description-input')).toHaveValue('Descripción del producto de prueba que es lo suficientemente larga')
  ...
  describe('Validación de formulario', () => {
    ...
    it('muestra error cuando el estado es inválido', async () => {
      renderWithRecoil(<EditItemModal {...defaultProps} />)
  ...
  describe('Manejo de imágenes', () => {
    it('muestra mensaje de error al intentar subir un archivo que no es imagen', async () => {
      ...
      expect(screen.getByTestId('error-image')).toHaveTextContent('Por favor, selecciona una imagen válida')
    })
    ...
    it('muestra mensaje de error al intentar subir una imagen mayor a 5MB', async () => {
      ...
      expect(screen.getByTestId('error-image')).toHaveTextContent('La imagen es demasiado grande. El tamaño máximo es 5MB.')
    })
  ...
  describe('Manejo de estado y loading', () => {
    it('muestra el estado de loading durante la validación', async () => {
      renderWithRecoil(<EditItemModal {...defaultProps} />)
    ...
    it('resetea el formulario al cerrar', async () => {
      renderWithRecoil(<EditItemModal {...defaultProps} card={{
        ...mockCard,
        title: "Producto de prueba"
      }} />)
  ...
  describe('Manejo de cambios en campos', () => {
    it('actualiza la cantidad a 0 cuando el estado cambia a inactivo', async () => {
      renderWithRecoil(<EditItemModal {...defaultProps} />)

      const statusSelect = screen.getByTestId('status-value')
  ...
  describe('Manejo de acciones', () => {
    it('llama a onSave con los datos actualizados al guardar', async () => {
      ...
      fireEvent.change(screen.getByTestId('title-input'), { target: { value: 'Nuevo título' } })
      fireEvent.change(screen.getByTestId('description-input'), { target: { value: 'Nueva descripción larga para el producto' } })
